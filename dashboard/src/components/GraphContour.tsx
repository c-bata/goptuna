...
export const Contour: FC<{
  ...
  useEffect(() => {
    if (study != null) {
      plotContour(study, xParam, yParam, theme.palette.mode)
    }
  }, [study, xParam, yParam, theme.palette.mode])
...
const filterFunc = (trial: Trial): boolean => {
  return trial.state === "Complete" && trial.value !== undefined
}
...
const plotContour = (
  ...
  filteredTrials.forEach((trial, i) => {
    if (xAxis.values[i] && yAxis.values[i] && trial.value) {
  ...
  const plotData: Partial<plotly.PlotData>[] = [
    {
      ...
      reversescale: study.direction !== "minimize",
